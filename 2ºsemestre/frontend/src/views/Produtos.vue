...
function mapearProdutoApiParaTela(produtoApi) {
  return {
    id: produtoApi.id,
    nome: produtoApi.nome,
    preco: Number(produtoApi.preco) || 0,
    quantidade: Number(produtoApi.estoque) || 0,
  }
}
...
async function carregarProdutos() {
  ...
  mensagemErro.value = ''
  try {
    const listaApi = await listarProdutosApi()
    produtos.value = (listaApi || []).map(mapearProdutoApiParaTela)
  } catch (error) {
